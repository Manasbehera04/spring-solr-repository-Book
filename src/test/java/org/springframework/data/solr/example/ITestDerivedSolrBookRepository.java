...
import org.springframework.data.solr.example.repository.BookRepo;
import org.springframework.data.solr.example.repository.BookRepoImpl;
import org.springframework.data.solr.example.repository.CustomSolrRepositoryImpl;
import org.springframework.data.solr.example.repository.DerivedSolrBookRepository;
...
public class ITestDerivedSolrBookRepository extends AbstractSolrIntegrationTest {

	@Autowired
	SolrOperations solrOperations;

	private DerivedSolrBookRepository repo;

	@Before
	public void setUp() {
		// Create new repository instance using Factory and inject custom implementation
		repo = new SolrRepositoryFactory(this.solrOperations).getRepository(DerivedSolrBookRepository.class,
				new BookRepoImpl(this.solrOperations));
		//repo= new SolrRepositoryFactory(this.solrOperations).getRepository(DerivedSolrBookRepository.class);
	}

	@After
	public void tearDown() {
		/*solrOperations.delete("jcg",new SimpleQuery(new SimpleStringCriteria("*:*")));
		solrOperations.commit("jcg");*/
	}
...
//solrOperations.saveBean("jcg", rrs);
//Assert.assertEquals(1, repo.count());
		Page<Book> page = repo.findBooksByCustomImplementation("book1", new PageRequest(0, 10));
